              ...
              <Card key={idx} className="bg-neutral-900 border-0">
                <CardContent className="p-6">
                        ...
                        return (
                          <div
                            ...
                            className={`p-2 rounded border flex items-center gap-2
                              ${isCorrect ? "border-green-500 bg-green-900/30" : "border-neutral-700"}
                              ${isSelected && !isCorrect ? "border-blue-500 bg-blue-900/30" : ""}
                              ${isSelected && isCorrect ? "border-green-700 bg-green-800/60" : ""}
                            `}
                          >
                            <span className={`font-semibold ${isCorrect ? "text-green-400" : isSelected ? "text-blue-400" : "text-white"}`}>
                              {option}
                            </span>
                            {isCorrect && <span className="ml-2 text-xs text-green-400">(Correcta)</span>}
                            {isSelected && !isCorrect && <span className="ml-2 text-xs text-blue-400">(Tu respuesta)</span>}
                            {isSelected && isCorrect && <span className="ml-2 text-xs text-green-400">(Tu respuesta correcta)</span>}
                          </div>
                        );
                      })}
                    </div>
                  ...
                  <div className="space-y-3">
                    <div>
                      <div className="text-sm text-slate-400 mb-1">Tu respuesta:</div>
                      <div className="text-white bg-neutral-800 p-3 rounded">
                    ...
                    {!answer?.isCorrect && (
                      <div>
                        <div className="text-sm text-slate-400 mb-1">Respuesta correcta:</div>
